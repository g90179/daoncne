...
import api from '../../api/axios'; // 🔑 통합된 API 모듈 사용
...
const ResetPassword = () => {
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      // 🔑 api 인스턴스를 사용하여 호출 (API_URL 제거 및 통합 인스턴스 적용)
      await api.post('/auth/reset-password', {
        email, instanceKey, newPassword, robotToken
      });
      alert('비밀번호가 성공적으로 변경되었습니다.');
      
      // 메인 페이지로 이동하면서 로그인 팝업 활성화
        ...
        <form onSubmit={handleResetPassword} className="space-y-4 text-left">
          ...
          <div className="space-y-1">
            ...
            <input 
              ...
              placeholder="대소문자 숫자 조합" 
              className="w-full bg-slate-50/60 border border-slate-200/50 rounded-2xl px-5 py-3 text-sm text-[oklch(0.38_0.07_259.56)] font-mono outline-none focus:bg-white focus:border-blue-400 transition-all" 
            />
          </div>
          ...
          <div className="space-y-1">
            ...
            <input 
              ...
              placeholder="새 암호 입력" 
              className="w-full bg-slate-50/60 border border-slate-200/50 rounded-2xl px-5 py-3 text-sm text-[oklch(0.38_0.07_259.56)] outline-none focus:bg-white focus:border-blue-400 transition-all" 
            />
          </div>
